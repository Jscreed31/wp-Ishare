...
class Motu_Walkernav extends Walker_Nav_Menu
{
    ...
    public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
    {
        ...
        if( isset( $args->has_children ) && !$args->has_children && ( ( $child_cats && $hasMegaMenu_subcat ) || ( $hasMegaMenu && $mm_posts_query ) && $depth == 0 ) ) {
            ...
            $item_output .= '<span class="icon">'.motu_theme_svg('chevron-down',true).'</span>';
        ...
        $item_output .= ( ( $depth == 0 || 1 ) && $has_children ) ? '<span class="icon">'.motu_theme_svg('chevron-down',true).'</span></a>' : '</a>';
        ...
        if( $object_id && isset( $item->object ) && $item->object == 'category' && $depth == 0 && ( $hasMegaMenu || $hasMegaMenu_subcat ) ){
            ...
            if( ( $child_cats && $hasMegaMenu_subcat ) || ( $hasMegaMenu && $mm_posts_query ) ){
                if ($hasMegaMenu_subcat && $child_cats ) {
                    $mm_category_class = 'theme-megamenu-no-bg';
                }
                ...
                if( $hasMegaMenu_subcat && $child_cats ){
                    ...
                    $output .= '<li class="column column-3 column-sm-12 megamenu-content-left has-megamenu-content">';
                    ...
                    foreach( $child_cats as $child_cat ){
                        ...
                        $output .= '<span class="icon">'.motu_theme_svg('chevron-right',true).'</span>';
                        $output .= '</a>';
                        $output .= '</div>';
                ...
                if( $hasMegaMenu && $mm_posts_query ){
                    ...
                    $output .= '<li class="column column-sm-12 megamenu-content-right has-megamenu-content '. esc_html( $mega_menu_col_class ) .'">';
                    ...
                    foreach( $mm_posts_query as $mm_posts_content ){
                        ...
                        if( $mm_posts_content->ID ){
                            ...
                            $thumb_url = get_the_post_thumbnail($mm_posts_content->ID,'medium');
                            $post_date = get_the_date(get_option( 'date_format' ), $mm_posts_content->ID);
                        }
                        $output .= '<div class="megamenu-recent-article">';
                        $output .= '<div class="megamenu-article-image"><a href="'.esc_url( $url ).'">'.wp_kses_post($thumb_url).'</a><div class="megamenu-article-date">'.wp_kses_post($post_date).'</div></div>';
