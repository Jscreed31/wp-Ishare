...
if ( ! function_exists( 'featureon_dynamic_css' ) ) {
	...
	function featureon_dynamic_css() {
		if ( get_theme_mod('dynamic-styles', 'on') == 'on' ) {
			...
			// primary color
			if ( get_theme_mod('color-1','#0fb4d2') != '#0fb4d2' ) {
				$styles .= '
...
#wrap-nav-topbar .nav-menu.mobile > div > ul { border-top: 1px solid rgba(255,255,255,0.12); }
			...
			if ( get_theme_mod('color-header','') != '' ) {
				$styles .= '
#header-top { background: '.esc_attr( get_theme_mod('color-header') ).'; border-bottom: 1px solid rgba(255,255,255,0.2) }
.site-title a,
...
.toggle-search { color: #fff; border-left-color: rgba(255,255,255,0.2); border-right-color: rgba(255,255,255,0.2); }
			...
			if ( get_theme_mod('color-header-menu','') != '' ) {
				$styles .= '
#header-bottom,
#header-bottom.fixed { background: '.esc_attr( get_theme_mod('color-header-menu') ).'; }
...
@media only screen and (max-width: 719px) {
	...
	.toggle-search.active .svg-icon { fill: #333; }
			...
			// subheader color
			if ( get_theme_mod('color-subheader','') != '' ) {
				$styles .= '
#subheader,
.owl-carousel,
.page-title strong { background: '.esc_attr( get_theme_mod('color-subheader') ).'; }
#subheader,
.owl-carousel,
.featured-large,
.featured-large:nth-child(1),
.featured-small { border-color: '.esc_attr( get_theme_mod('color-subheader') ).'; }
.page-title span { color: rgba(255,255,255,0.8); }
.page-title { border-color: rgba(255,255,255,0.3); }
@media only screen and (max-width: 719px) {
	.featured-item { border-color: '.esc_attr( get_theme_mod('color-subheader') ).'; }
}
				'."\n";
			}
			// footer menu color
...
#wrap-nav-footer .nav-menu:not(.mobile) .menu ul ul:after { border-right-color: '.esc_attr( get_theme_mod('color-footer-menu') ).'; border-top-color: transparent; }	
			...
			if ( get_theme_mod('featured-height','540') != '540' ) {
				$styles .= '.featured-posts-inner { height: '.esc_attr( get_theme_mod('featured-height') ).'px; }'."\n";
			}
			// image border radius
